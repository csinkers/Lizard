...
public class Duplex
{
    ...
    async Task Outbound(Stream stream, CancellationToken ct = default)
    {
        while (!ct.IsCancellationRequested)
        {
            ...
            _log.LogInformation("SEND {id} {type} - {size} bytes", packet.Id, packet.Type, packet.Data.Length);
    ...
    async Task<Packet?> ReceivePacketAsync(Stream stream, byte[] header, CancellationToken ct)
    {
        ...
        _log.LogInformation("RECV {id} {type} - {size} bytes", id, packetType, packetSize);
